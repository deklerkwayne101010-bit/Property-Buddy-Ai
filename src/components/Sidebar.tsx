...
export default function Sidebar({ isOpen, onToggle }: SidebarProps) {
  ...
  const menuItems: MenuItem[] = [
    ...
    { href: '/properties', label: 'My Properties', icon: '🏠' },
    ...
    <>
      ...
      >
        ...
        <nav className="relative flex flex-col flex-1">
          {/* Menu items */}
          <ul className="space-y-1 sm:space-y-2 px-3 sm:px-6 py-3 sm:py-6 flex-shrink-0">
                      ...
                      <ul className="ml-6 mt-2 space-y-1 animate-fade-in">
                        {item.children.map((child, childIndex) => (
                          <li key={child.href} className="animate-fade-in" style={{ animationDelay: `${(index + childIndex + 1) * 0.1}s` }}>
                            <Link
                              href={child.href!}
                              className={`group relative flex items-center px-3 sm:px-4 py-3 rounded-lg transition-all duration-300 ease-out transform hover:scale-[1.02] focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:ring-offset-2 focus:ring-offset-slate-900 overflow-hidden ${
                                pathname === child.href
