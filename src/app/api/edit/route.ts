...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.log('=== Processing failed, refunding credits ===');
      try {
        const refundResult = await addCredits(userId, 1, 'photo_edit_refund');
        if (refundResult.success) {
          console.log(`Successfully refunded 1 credit to user ${userId}. New balance: ${refundResult.newCredits}`);
        } else {
