...
export default function BuyerPackMakerPage() {
  ...
  const handleScrapeProperty = async () => {
    ...
    if (isProcessing) {
      console.log('Already processing a job, ignoring duplicate request');
      return;
    }
        ...
        >
          ...
          <div className="bg-white rounded-lg shadow-md p-6 mb-8">
            <div className="flex items-center justify-between mb-6">
              ...
              {scrapedData.length > 0 && !isProcessing && (
              ...
              {scrapedData.length > 0 && !isProcessing && scrapedData.length < 10 && (
                <button
              ...
              {scrapedData.length >= 10 && !isProcessing && (
                <div className="bg-orange-50 border border-orange-200 rounded-lg px-4 py-2">
                  <div className="text-sm text-orange-700 font-medium">
                    Maximum Properties Reached (10)
                  </div>
                </div>
              )}
            </div>

            {/* Quick Add Section - Only show when not processing */}
            {showAddForm && !isProcessing && (
